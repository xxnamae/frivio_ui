...
| **Maskinell håndheving av `regel/700-er-flate-900-er-forgrunn` per kallested** | `scripts/kontrastmaal.mjs` måler kun TOKEN-PAR i `globals.css`, ikke faktisk brukte farge/element-kombinasjoner i `.tsx`. Go-live-revisjonen 2026-09-24 (Vedlegg D) fant nettopp denne regelen brutt i praksis (`HELSE_FARGE.lav = var(--color-success)` som tekstfarge, 2,92:1) — en regel som fantes fra før, men ikke håndheves av verktøy. En enkel grep etter `text-(color:--color-success\|warning\|error)` ville truffet MANGE lovlige treff (regelens eget unntak for ikoner/fylte flater bruker nøyaktig samme klassenavn), så en treffsikker sjekk krever å skille «tekst»-bruk fra «ikon»-bruk — ikke gjort med et regex-grep alene. | Noen med tid til å bygge en AST-basert (ikke regex) sjekk som ser på ELEMENTTYPEN klassen står på (tekstnode/`<span>`/`<p>` vs. et Icon-komponentkall), eller en enklere mellomting: en baseline-fil (samme mønster som `designvakt-baseline.json`) som fanger NYE forekomster uten å kreve at alle eksisterende ikon-bruk migreres først. |
